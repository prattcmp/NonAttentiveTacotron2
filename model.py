...
class Duration(nn.Module):
    def __init__(self, hparams):
        ...
        self.encoder_output_dim = hparams.encoder_output_dim + hparams.speaker_embedding_dim + hparams.torchmoji_embedding_dim #+ hparams.gst_embedding_dim
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.n_mel_channels = hparams.n_mel_channels
        self.n_frames_per_step = hparams.n_frames_per_step
        self.torchmoji_embedding_dim = hparams.torchmoji_embedding_dim
        ...
        #self.speaker_projection = nn.Linear(hparams.speaker_input_dim, hparams.speaker_embedding_dim, bias=True)
        #self.tpse = TPSE(hparams)
        self.duration_aligner = Duration(hparams)
        self.decoder = Decoder(hparams)
        self.postnet = Postnet(hparams)

    def parse_batch(self, batch, speaker_embeddings, torchmoji_encodings, teacher_forcing=False):
        text_padded, input_lengths, mel_padded, duration_padded, \
            output_lengths, audio_names, text_strings, mels2 = batch
        ...
        new_torchmoji_encodings = np.zeros(shape=(torchmoji_encodings.shape[0], self.torchmoji_embedding_dim))
        for i in range(torchmoji_encodings.shape[0]):
            encoding = np.copy(torchmoji_encodings[i])
            k = self.torchmoji_embedding_dim
            ind = np.argpartition(encoding, -k)[-k:]
            # 2304 - scale down values
            new_torchmoji_encodings[i] = ind[np.argsort(encoding[ind])][::-1] / 2304.

        torchmoji_encodings = to_gpu(torch.from_numpy(new_torchmoji_encodings)).float()

        if teacher_forcing:
            return (
                (text_padded, input_lengths, mel_padded, duration_padded, max_len, output_lengths, speaker_embeddings),
                (mel_padded, duration_padded), audio_names)

        return (
            (text_padded, input_lengths, mel_padded, duration_padded, max_len, output_lengths, speaker_embeddings, torchmoji_encodings),
    ...
    def forward(self, inputs):
        text_inputs, text_lengths, mels, durations, max_len, output_lengths, speaker_embeddings, torchmoji_encodings = inputs
        ...
        # Concatenate speaker embedding to encoder outputs
        #speaker_embeddings = self.speaker_projection(speaker_embeddings)
        speaker_embeddings = speaker_embeddings.unsqueeze(1).expand(-1, encoder_outputs.size(1), -1)

        torchmoji_encodings = torchmoji_encodings.unsqueeze(1).expand(-1, encoder_outputs.size(1), -1)

        encoder_outputs_cat = torch.cat((encoder_outputs, speaker_embeddings, torchmoji_encodings), -1)
    ...
    def inference(self, inputs):
        text_inputs, speaker_embeddings, torchmoji_encodings = inputs
        embedded_inputs = self.embedding(text_inputs).transpose(1, 2)
        ...
        torchmoji_encodings = torchmoji_encodings.unsqueeze(0).unsqueeze(1).expand(-1, encoder_outputs.size(1), -1)
